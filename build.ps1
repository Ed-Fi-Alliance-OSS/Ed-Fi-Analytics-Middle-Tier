...
function CleanPublishFddOutputDirectory {
    if (Test-Path -Path $publishFddOutputDirectory) {
        Remove-Item -Recurse (JOIN-PATH $publishFddOutputDirectory *.*)
    }
    if (Test-Path -Path (JOIN-PATH $publishOutputPath $publishFddZipFile)) {
        Remove-Item -Recurse (JOIN-PATH $publishOutputPath $publishFddZipFile)
    }
}

function CleanPublishScdOutputDirectory {
    if (Test-Path -Path $publishScdOutputDirectory) {
        Remove-Item -Recurse (JOIN-PATH $publishScdOutputDirectory *.*)
    }
    if (Test-Path -Path (JOIN-PATH $publishOutputPath $publishScdZipFile)) {
        Remove-Item -Recurse (JOIN-PATH $publishOutputPath $publishScdZipFile)
    }
}
...
function AssemblyInfo {
    Invoke-Execute {
        ...
        Invoke-RegenerateFile (JOIN-PATH $solutionRoot "Directory.Build.props") @"
<Project>
