...
AS
    SELECT
        ...
        CONCAT(Grade.GradingPeriodDescriptorId, '-', Grade.SchoolId, '-', to_char(GradingPeriod.BeginDate, 'yyyymmdd'))
            AS GradingPeriodKey,
        CONCAT(Student.StudentUniqueId, '-', Grade.SchoolId, '-', Grade.LocalCourseCode, '-', Grade.SchoolYear, '-',
            Grade.SectionIdentifier, '-', Grade.SessionName, '-', to_char(Grade.BeginDate, 'yyyymmdd')) AS StudentSectionKey,
        CONCAT(Grade.SchoolId, '-', Grade.LocalCourseCode, '-', Grade.SchoolYear, '-', Grade.SectionIdentifier, '-', 
            Grade.SessionName) AS SectionKey,
        COALESCE(Grade.NumericGradeEarned, ews_LetterGradeTranslation.NumericGradeEarned) AS NumericGradeEarned,
    ...
    WHERE
        DescriptorConstant.ConstantName IN('GradeType.GradingPeriod', 'GradeType.Semester', 'GradeType.Final');
