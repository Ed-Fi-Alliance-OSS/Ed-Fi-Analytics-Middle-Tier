...
CREATE OR REPLACE VIEW analytics.StaffSectionDim
AS
     SELECT 
		CONCAT(s.StaffUniqueId,'-',CAST(StaffSectionAssociation.SchoolId AS VARCHAR), '-' , StaffSectionAssociation.LocalCourseCode , '-' , CAST(StaffSectionAssociation.SchoolYear AS VARCHAR) , '-' , StaffSectionAssociation.SectionIdentifier , '-' , StaffSectionAssociation.SessionName) AS StaffSectionKey
		,s.StaffUSI
		,s.StaffUniqueId as UserKey
		,CAST(StaffSectionAssociation.SchoolId AS VARCHAR) AS SchoolKey
		,CONCAT(CAST(StaffSectionAssociation.SchoolId AS VARCHAR), '-' , StaffSectionAssociation.LocalCourseCode , '-' , CAST(StaffSectionAssociation.SchoolYear AS VARCHAR) , '-' , StaffSectionAssociation.SectionIdentifier , '-' , StaffSectionAssociation.SessionName) AS SectionKey
	...
	WHERE StaffSectionAssociation.EndDate IS NULL OR StaffSectionAssociation.EndDate > NOW();
