...
AS
    WITH ParentAddress
         AS (SELECT 
                    ...
                    CONCAT_WS(' ',
                        CONCAT_WS(', ',
                        ParentAddress.StreetNumberName, 
                        ParentAddress.ApartmentRoomSuiteNumber, 
                        ParentAddress.City), 
                        StateAbbreviationType.CodeValue, 
                        ParentAddress.PostalCode) AS Address, 
             ...
             FROM 
                  edfi.ParentAddress
             INNER JOIN
                 edfi.Descriptor AS AddressType ON
             ...
             INNER JOIN
                 edfi.Descriptor AS StateAbbreviationType ON
         ...
         AS (SELECT 
             ...
             INNER JOIN
                 edfi.Descriptor AS TelephoneNumberType ON
         ...
         AS (SELECT 
             ...
             LEFT OUTER JOIN
                 edfi.Descriptor AS HomeEmailType ON
         ...
         SELECT 
                ...
                CASE WHEN
                 HomeEmail.PrimaryEmailAddressIndicator = 1
                    THEN 'Personal' WHEN
                 WorkEmail.PrimaryEmailAddressIndicator = 1
                    THEN 'Work'
                    ELSE 'Not specified'
                ...
                ( SELECT 
                  ...
                  FROM
                       (VALUES(StudentParentAssociation.LastModifiedDate), (Parent.LastModifiedDate), (HomeAddress.LastModifiedDate), (PhysicalAddress.LastModifiedDate), (MailingAddress.LastModifiedDate), (WorkAddress.LastModifiedDate), (TemporaryAddress.LastModifiedDate), (HomeTelephone.CreateDate), (MobileTelephone.CreateDate), (WorkTelephone.CreateDate), (HomeEmail.CreateDate), (WorkEmail.CreateDate)) AS VALUE(MaxLastModifiedDate)
                ) AS LastModifiedDate
         FROM 
