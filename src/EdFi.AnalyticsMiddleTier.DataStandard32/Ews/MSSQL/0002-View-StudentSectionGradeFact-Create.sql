...
AS
    SELECT
        ...
        CONCAT(Grade.GradingPeriodDescriptorId, '-', Grade.SchoolId, '-', CONVERT(NVARCHAR, GradingPeriod.BeginDate, 112))
            AS GradingPeriodKey,
        CONCAT(Student.StudentUniqueId, '-', Grade.SchoolId, '-', Grade.LocalCourseCode, '-', Grade.SchoolYear, '-',
            Grade.SectionIdentifier, '-', Grade.SessionName, '-', CONVERT(NVARCHAR, Grade.BeginDate, 112)) AS StudentSectionKey,
    ...
    WHERE
        DescriptorConstant.ConstantName = 'GradeType.GradingPeriod';