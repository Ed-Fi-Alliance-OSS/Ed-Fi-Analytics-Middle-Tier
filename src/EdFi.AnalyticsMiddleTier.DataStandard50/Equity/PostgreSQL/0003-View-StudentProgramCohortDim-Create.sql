...
AS
	SELECT
		CONCAT(
			...
			'-',
			TO_CHAR(StudentCohortAssociation.BeginDate, 'yyyymmdd'),
			'-',
	...
	INNER JOIN
		edfi.Program ON
			CohortProgram.ProgramEducationOrganizationId = program.EducationOrganizationId
	...
	INNER JOIN 
		edfi.StudentCohortAssociation ON 
	...
	INNER JOIN 
		edfi.Student ON 
    StudentCohortAssociation.StudentUSI = Student.StudentUSI
	INNER JOIN 
