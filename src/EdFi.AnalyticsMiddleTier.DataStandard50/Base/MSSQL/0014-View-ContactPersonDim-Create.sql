...
AS
    WITH ContactAddress AS (
        SELECT
            ...
            CONCAT(COALESCE(ContactAddress.StreetNumberName, ''), COALESCE(', ' + ContactAddress.ApartmentRoomSuiteNumber, ''), 
                COALESCE(', ' + ContactAddress.City, ''), COALESCE(' ' + StateAbbreviationType.CodeValue, ''), 
                COALESCE(' ' + ContactAddress.PostalCode, '')) AS Address,
        ...
        ON
            ContactAddress.AddressTypeDescriptorId = AddressType.DescriptorId
        INNER JOIN
        ...
        ON
            ContactAddress.AddressTypeDescriptorId = ContactAddressPeriod.AddressTypeDescriptorId
        AND
            ContactAddress.ContactUSI = ContactAddressPeriod.ContactUSI
        INNER JOIN
            edfi.Descriptor AS StateAbbreviationType
        ON
            ContactAddress.StateAbbreviationDescriptorId = StateAbbreviationType.DescriptorId
        WHERE
            ContactAddressPeriod.EndDate IS NULL
        OR
            ContactAddressPeriod.EndDate > GETDATE()
    ), ContactTelephone AS (
        SELECT
            ContactTelephone.ContactUSI,
        ...
        ON
            ContactTelephone.TelephoneNumberTypeDescriptorId = TelephoneNumberType.DescriptorId
        INNER JOIN
            analytics_config.DescriptorMap
        ON
        ...
        SELECT
            ContactElectronicMail.ContactUSI,
        ...
        ON
            ContactElectronicMail.ElectronicMailTypeDescriptorId = HomeEmailType.DescriptorId
        LEFT JOIN
            analytics_config.DescriptorMap
        ON
    ...
    SELECT
        CONCAT(Contact.ContactUniqueId,'-',Student.StudentUniqueId) AS UniqueKey,
        ...
        COALESCE(StudentContactAssociation.PrimaryContactStatus, CAST(0 as BIT)) AS IsPrimaryContact,
        COALESCE(StudentContactAssociation.LivesWith, CAST(0 as BIT)) AS StudentLivesWith,
        COALESCE(StudentContactAssociation.EmergencyContactStatus, CAST(0 as BIT)) AS IsEmergencyContact,
    ...
    INNER JOIN
        edfi.Student
    ON
        StudentContactAssociation.StudentUSI = edfi.Student.StudentUSI
    INNER JOIN
        edfi.Contact
    ON
        StudentContactAssociation.ContactUSI = Contact.ContactUSI
    INNER JOIN
        edfi.Descriptor AS RelationType
    ON
        StudentContactAssociation.RelationDescriptorId = RelationType.DescriptorId
    LEFT OUTER JOIN
        ContactAddress AS HomeAddress
    ON
        Contact.ContactUSI = HomeAddress.ContactUSI
    AND
        HomeAddress.ConstantName = 'Address.Home'
    LEFT OUTER JOIN
        ContactAddress AS PhysicalAddress
    ON
        Contact.ContactUSI = PhysicalAddress.ContactUSI
    AND
        PhysicalAddress.ConstantName = 'Address.Physical'
    LEFT OUTER JOIN
        ContactAddress AS MailingAddress
    ON
        Contact.ContactUSI = MailingAddress.ContactUSI
    AND
        MailingAddress.ConstantName = 'Address.Mailing'
    LEFT OUTER JOIN
        ContactAddress AS WorkAddress
    ON
        Contact.ContactUSI = WorkAddress.ContactUSI
    AND
        WorkAddress.ConstantName = 'Address.Work'
    LEFT OUTER JOIN
        ContactAddress AS TemporaryAddress
    ON
        Contact.ContactUSI = TemporaryAddress.ContactUSI
    AND
        TemporaryAddress.ConstantName = 'Address.Temporary'
    LEFT OUTER JOIN
        ContactTelephone AS HomeTelephone
    ON
        Contact.ContactUSI = HomeTelephone.ContactUSI
    AND
        HomeTelephone.ConstantName = 'Telephone.Home'
    LEFT OUTER JOIN
        ContactTelephone AS MobileTelephone
    ON
        Contact.ContactUSI = MobileTelephone.ContactUSI
    AND
        MobileTelephone.ConstantName = 'Telephone.Mobile'
    LEFT OUTER JOIN
        ContactTelephone AS WorkTelephone
    ON
        Contact.ContactUSI = WorkTelephone.ContactUSI
    AND
        WorkTelephone.ConstantName = 'Telephone.Work'
    LEFT OUTER JOIN
        ContactEmail AS HomeEmail
    ON
        Contact.ContactUSI = HomeEmail.ContactUSI
    AND
        HomeEmail.ConstantName = 'Email.Personal'
    LEFT OUTER JOIN
        ContactEmail AS WorkEmail
    ON
        Contact.ContactUSI = WorkEmail.ContactUSI
    AND
