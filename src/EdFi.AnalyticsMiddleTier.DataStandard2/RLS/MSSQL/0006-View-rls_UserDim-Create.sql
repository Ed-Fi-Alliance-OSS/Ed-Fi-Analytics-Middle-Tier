...
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = 'analytics' AND TABLE_NAME = 'rls_UserDim')
BEGIN
...
AS
     ...
     INNER JOIN
         ...
         ON StaffElectronicMail.ElectronicMailTypeId = ElectronicMailType.ElectronicMailTypeId
     INNER JOIN
         analytics_config.TypeMap
         ON ElectronicMailType.ElectronicMailTypeId = TypeMap.TypeId
     INNER JOIN
